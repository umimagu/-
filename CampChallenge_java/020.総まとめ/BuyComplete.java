...
public class BuyComplete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int how2send = Integer.parseInt(request.getParameter("how2send"));
            UserDataDAO.getInstance().insertItems(dto, how2send);

            dto.clearCart();
            
            //セッションから合計金額を取得しユーザ情報の総購入金額をアップデート
            int total = Integer.parseInt(String.valueOf(hs.getAttribute("total")));
            dto.setTotal(dto.getTotal() + total);
            UserDataDAO.getInstance().updateTotal(dto);
